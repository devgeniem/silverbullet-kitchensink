...
export default (dispatch) => {
  const deleteList = id => api.del('/todo-list/'+id).then(response => dispatch({ type: Types.REMOVE_LIST, response }));

  const createList = data => {
    var promise = api.post('/todo-list', { title: data.title, items: JSON.stringify(data.items) })
      .then(response => dispatch({ type: Types.CREATE_LIST, response }));
    return promise;
    }

  const modifyList = (id, data) => api.put('todo-list/'+id, { title: data.title, items: JSON.stringify(data.items) }).then(() => dispatch({ type: Types.MODIFY_LIST }));
