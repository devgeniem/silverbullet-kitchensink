...
//import s from './add-new-order.scss';
import api from '../../services/api';
import { connect } from "react-redux";
import { getCurrencySymbol, formatThousands, removeItemByPropVal } from '../../services/utils';
...
class AddNewOrder extends React.Component {
    ...
    handleSubmit(e) {
        ...
        api.post('/order', this.state.formData)
            .then(() => {
                this.props.addNotification('success', 'New order created');
    ...
    handleFormValueChange(field, data) {
        ...
        if ((field === 'billing' || field === 'estimatedHours') && (formData.estimatedHours && formData.billing)) {
            totalBudget = formData.billing.toString().replace(/\s+/g, "") * formData.estimatedHours;
        }
        ...
        return (
            <div className={s.root}>
                ...
                <Form onSubmit={e => this.handleSubmit(e)} horizontal>
                    ...
                    <FormGroup controlId="state" validationState={validationState.state}>
                        ...
                        <Col sm={6}>
                            <FormControl componentClass="select" onChange={e => this.handleFormValueChange('state', e.target.value)}>
                                <option key="state-default-option" value="">Select order state</option>
                                {formOptions && formOptions.orderStates ? formOptions.orderStates.map((val, i) => <option key={'state-option-'+i} value={val}>{val}</option>) : null}
                            </FormControl>
                            <FormControl.Feedback />
                        </Col>
                    </FormGroup>
                    ...
                    <FormGroup controlId="estimatedHours" validationState={validationState.estimatedHours}>
                        <Col componentClass={ControlLabel} sm={2}>
                            Work estimate
                        </Col>
                        <Col sm={6}>
                            <FormControl type="text" value={formData.estimatedHours} placeholder="Estimate in hours" onChange={e => this.handleFormValueChange('estimatedHours', e.target.value)} />
                            <FormControl.Feedback />
                    ...
                    <FormGroup>
                        <Col smOffset={2} sm={6}>
                            {totalBudget ? <p><strong>{formatThousands(formData.billing)}</strong> {getCurrencySymbol(formOptions, formData.currency)}/h x <strong>{formData.estimatedHours}</strong> h = <strong>{formatThousands(totalBudget)}</strong> {getCurrencySymbol(formOptions, formData.currency)}</p> : null }
                        </Col>
                    </FormGroup>
                    ...
                    <FormGroup controlId="invoicer" validationState={validationState.invoicer}>
                        ...
                        <Col sm={6}>
                            <FormControl componentClass="select" onChange={e => this.handleFormValueChange('invoicer', e.target.value)}>
                                <option key="invoicer-default-option" value="">Select user managing invoicing</option>
                                {users.map(val => <option key={'invoicer-option-'+val.id} value={val.id}>{val.firstname} {val.lastname}</option>)}
                            </FormControl>
                            <FormControl.Feedback />
                        </Col>
                    </FormGroup>

                    <Button type="submit" disabled={this.state.submitDisabled} bsSize="large" bsStyle="primary" className="pull-right">Add order</Button>
